...
STOCK_TERMS = ("stock", "share", "equity", "nse", "bse", "market cap", "p/e", "pe ratio", "52 week", "52-week", "dividend", "fundamental", "fundamentals", "technical analysis", "analyze", "analyse", "buy", "sell")
...
class AIAgent:
    ...
    async def _ollama(self, prompt: str) -> str:
        payload = {"model": self.model_name, "prompt": prompt, "stream": False, "options": {"temperature": 0.1, "top_p": 0.9, "num_predict": 384, "stop": ["USER:", "\nUSER:"]}}
        ...
        if intent == "stock":
            ...
            if not resolved.get("success"):
                if resolved.get("ambiguous"):
                    names = ", ".join(f"{c['company_name']} ({c['ticker']})" for c in resolved.get("candidates", []))
                    return context, None, tools_used, f"I found multiple possible matches: {names}. Please provide the exact NSE/BSE ticker."
                return context, None, tools_used, resolved.get("error", "I couldn't reliably identify the NSE/BSE stock.")
